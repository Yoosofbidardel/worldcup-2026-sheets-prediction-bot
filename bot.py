...
ADMIN_GUIDE = (
    "\n\n— — — — —\n"
    "👑 *ویژه‌ی ادمین*\n"
    "• «📢 پیام به همه» — یه پیام برای همه‌ی شرکت‌کننده‌ها بفرست (تو پیویِ هرکس).\n"
...
async def stepper_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match.get("knockout") and home == away:
        ...
        await query.answer("نتیجه ذخیره شد ✅")
        ...
        await _edit(
            ...
            "⚖️ مساوی پیش‌بینی کردی! کدوم تیم تو پنالتی صعود می‌کنه؟ 👇",
            reply_markup=kb,
        )
        return
...
async def penalty_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not match or not match["open"]:
        await query.answer()
        await _edit(query, "⏰ این بازی همین الان بسته شد — پنالتی ذخیره نشد. 😬")
        return
    home_adv = side == "h"
...
async def cmd_sendgroup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not store.get_group_id():
        await _say(update, "⚠️ هنوز هیچ گروهی ثبت نشده. اول داخل خودِ گروه `/setgroup` رو بزن.")
        return
...
async def _post_to_group(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if not gid:
        await _say(update, "⚠️ هنوز هیچ گروهی ثبت نشده. اول داخل گروه `/setgroup` رو بزن.")
        return
    try:
        ...
        await _say(update, "✅ پیامت تو گروه گذاشته شد. 📨")
    except Exception as e:
        logging.warning("Group post failed: %s", e)
        await _say(update, "❌ نشد بفرستم تو گروه. مطمئن شو ربات تو گروهه و اجازه‌ی ارسال داره.")
...
async def cmd_syncknockout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not _is_admin(update.effective_user.id):
        return
    await _say(update, "⏳🔄 دارم بازی‌های جدید مرحله حذفی رو از API می‌گیرم…")
    report = await _sync_knockout(context)
    if not report["teams"] and not report["kickoffs"]:
        await _say(update, "🤷 بازی جدیدی نبود که اضافه بشه (تیم‌های دور بعد هنوز مشخص نشدن).")
        return
    shown = "\n".join(
    ...
    await _say(
        update,
        f"✅ *{_fa_num(report['teams'])}* بازی جدید اضافه شد و *{_fa_num(report['kickoffs'])}* زمان شروع نوشته شد. ⚽️"
        + (f"\n{shown}" if shown else ""),
    )
...
def _gainers_message(items: list) -> str:
    ...
    for i, it in enumerate(items):
        ...
        lines.append(
            ...
            f"دیروز *{_iso(_fmt_total(it['points']))}* امتیاز گرفت 📈"
        )
    lines.append(
...
async def cmd_topgainers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not items:
        await _say(update, "🤔 هنوز گزارشِ روزانه‌ای ثبت نشده. اولین گزارش فردا ساعت ۹ صبح خودکار میاد و بعد می‌تونی همین‌جا دوباره بفرستیش.")
        return
    try:
        await context.bot.send_message(
        ...
        await _say(update, "✅ نفراتِ برترِ دیروز تو گروه اعلام شدن. 🏅")
    except Exception as e:
