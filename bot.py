...
async def special_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row == CHAMPION_ROW and _deadline_passed():
        msg = (
            f"🏆 *{special['label']}*\n\n"
            "⚠️ مهلتِ قفلِ پیش‌بینی گذشتِس. می‌تونی عوضش کنی، ولی دیگه اون *۵ امتیازِ بونوسِ* قفل‌شدن بهت نمی‌رسه جونم.\n"
            "✍️ اسمِ تیمِ قهرمانو بنویس بفرست."
        )
    elif row == CHAMPION_ROW:
        msg = (
            f"🏆 *{special['label']}*\n\n"
            f"💡 اگه تا ددلاین ({_deadline_str()}) ثبتش کنی و دیگه عوضش نکنی، اگه درست دربیاد *۵ امتیازِ اضافه* می‌گیری‌ها! 🤑\n"
            "✍️ اسمِ تیمِ قهرمانو بنویس بفرست."
        )
    else:
...
async def cmd_mypredictions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # specials, except the champion (handled separately — two cells)
    ...
    # champion: effective pick (post-deadline change wins) + bonus status
    ...
    if eff:
        ...
        if live:
            ...
            status = "  🔒 (قفل شدِس تا ددلاین — واجدِ ۵ امتیاز بونوس ✅)"
        else:
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # top-scorer & best-player hard-close at the deadline (guard if it passed mid-typing)
    ...
    if row == CHAMPION_ROW and _champion_locked():
        context.user_data.clear()
        await _say(update, f"⏰ مهلتِ تغییرِ قهرمان تموم شدِس ({_final_deadline_str()}). دیگه قفله جونم. 🔒")
        return
    # champion after the first deadline -> SECOND cell (post-deadline change), no bonus
    ...
    if after:
        await _say(
            ...
            "⚠️ چون بعدِ ددلاین بود، اون ۵ امتیازِ بونوس بهت نمی‌رسه جونم.",
        )
    elif row == CHAMPION_ROW:
        await _say(
            update,
            f"✅ ثبت شد: {label}  →  *{_iso(text)}* 🏆\n"
            "🔒 اگه تا ددلاین دیگه عوضش نکنی و درست دربیاد، ۵ امتیازِ اضافه می‌گیری‌ها! 🤑",
        )
    else:
        await _say(
            update,
