...
async def special_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _special_is_open(special):
        await _edit(query, f"⏰ مهلتِ این پیش‌بینی تموم شدِس ({_deadline_str()}). دیگه نمی‌شه ثبتش کرد جونم. 🔒")
        return
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row in (BESTPLAYER_ROW, TOPSCORER_ROW) and _deadline_passed():
        ...
        return
    # champion after the deadline -> SECOND cell (post-deadline change), no bonus
